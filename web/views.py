    ...
    try:
        ...
        if is_best_ball:
            ...
            if is_dynasty:
                ...
                # For redraft best ball leagues, check league-configured position capacity limits
                print(f"DEBUG best ball (redraft): checking league position limits", file=sys.stderr)
                ...
                print(f"DEBUG best ball (redraft): o_count={o_count}, d_count={d_count}, g_count={g_count}", file=sys.stderr)
                
                if player_position == 'T':
                    # T players can move to O, D, or G - but check league's position limits
                    if o_count < league.roster_forwards:
                    ...
                    print(f"    T player: O={o_count}/{league.roster_forwards}, D={d_count}/{league.roster_defense}, G={g_count}/{league.roster_goalies}", file=sys.stderr)
                elif player_position == 'O':
                    # O players can stay in O position if there's an empty slot
                    if o_count < league.roster_forwards:
                        response_data['empty_slot_options']['O'] = ['O']
